...
6. **펀드/TDF 최적화 관리 및 UX 가이드 탑재 (v4.8.1)**
   - 수량과 무관하게 평가총액으로 기입하여 관리해야 하는 펀드/TDF 자산의 특성을 반영하여 수량을 1좌로 고정 및 평단가가 곧 현재 평가액이 되는 전용 계산식 분기를 구축했습니다.
   - 종목명 자동 감지 및 매수 폼 내 수동 자산 분류 라디오 버튼(`주식` / `펀드/TDF`)을 장착하고, 펀드 시세 갱신 시 종목코드 요구 대신 간편 평가액 수동 팝업 입력을 유도하여 갱신 과정을 단순화했습니다.
    - 주식 상세 내역 테이블 내에 `매수 평단가` 컬럼을 기본 지원합니다.
7. **거래 완료 시점 기준 역사적 보유 수량 계산 기능 (v4.11.0)**
    - 주식 거래 내역에서 거래 시점 당일 최종의 역사적 누적 보유 주수가 `*주 / *주` (거래 수량 / 누적 수량) 형태로 정밀 매핑되어, 과거 특정 시점의 보유 주수 추이를 실시간 현재 보유량과 무관하게 정확히 역추적하여 시각화합니다.
...
---
**Designed by 돼지지렁이 (PigWorm) v4.11.0**
---
